...
def studentSolution():
    files = [
        'network5.csv', 'network6.csv', 'network7.csv', 'network8.csv',
        'network19.csv'
    ]
    ...
    for i in range(1, 11, 2):
        ...
        try:
            ...
            print(
                "Runtime error on test {}, file: {}, function: shortestPaths('{}', adjList)"
                .format(test1, filename, name))
            return False
        except Exception as e:
            print(
                "Timeout error on test {}, file: {}, function: shortestPaths({}, adjList)"
                .format(test1, filename, name))
        ...
        try:
            ...
            print(
                "Timeout error on test {}, file: {}, function: betweennessCentrality"
                .format(test2, filename))
            return False
        except Exception as e:
            print(
                "Runtime error on test {}, file: {}, function: betweennessCentrality"
                .format(test2, filename))
            print("Error: {}".format(str(e)))
            return False
...
def main():
    ...
    res = requests.post(rootURI + "/advanced/pset3/centrality",
                        json=student_solutions)
    res = res.json()
