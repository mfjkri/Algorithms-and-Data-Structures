...
from min_heap1 import MinHeap1
...
def testConstructor(minHeap):
    ...
    print(cyanText("Initialising MinHeap1 with maxsize of 5"))

    try:
        title = "Test 1: MinHeap1 has correct attributes | "
        errMessage = '''Error: MinHeap1 class has incorrect attributes. Does it have heap, size, maxsize attributes? 
        ...
        assert (hasattr(minHeap, "heap")
                ...
                and hasattr(minHeap, "maxsize")), title + yellowText(errMessage)
...
Apple     | 15\n''' + color.END)
    ...
    try:
        ...
        errMessage = "Error: Incorrect ordering of heap items"
        ...
        print(cyanText("Inserting ('Microsoft', 100)"))
        minHeap.insert("Microsoft", 100)

        title = "Test 3: Testing heap remains unchanged once maxsize reached | "
        errMessage = "Error: heap should have remained unchange as maxsize reached"

        assert minHeap.size == 5, errMessage

        for i in range(1, 6):
            errMessage = "Error: Heap position {} should be {}, got {}. Heap should remain unchanged".format(
                i, order[i], minHeap.heap[i])
            assert (
                isinstance(minHeap.heap[i], HeapItem)
                and order[i].key == minHeap.heap[i].key
